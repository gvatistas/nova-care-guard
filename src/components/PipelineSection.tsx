  ...
  return (
    ...
    >
      <div
        ...
        style={{
          background: `radial-gradient(300px circle at ${glowPos.x}% ${glowPos.y}%, rgba(74,237,196,0.06), transparent 60%)`,
        }}
      />
      {children}
    </div>
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          background: `radial-gradient(800px circle at ${sectionMouse.x}% ${sectionMouse.y}%, rgba(74,237,196,0.03), transparent 50%)`,
        }}
      />
      ...
      <div className="relative max-w-[1400px] mx-auto px-6 md:px-8">
        ...
        {/* Visual pipeline nodes with 3D tilt */}
        <motion.div
          initial={{ opacity: 0 }}
          ...
          {stages.map((stage, i) => (
            <TiltCard key={i} className="relative z-10">
              ...
              >
                ...
                <span className={`font-mono text-xs mt-3 tracking-wide transition-colors duration-300 ${
                  hoveredStage === i ? "text-accent" : "text-gray-400"
                }`}>{stage.name}</span>
              </motion.div>
            </TiltCard>
          ))}
        ...
        <div className="border-t border-white/[0.06]">
          {stages.map((stage, i) => (
            <motion.div
              ...
              onMouseEnter={() => setHoveredStage(i)}
              onMouseLeave={() => setHoveredStage(null)}
              className={`group border-b border-white/[0.06] py-8 md:py-10 grid grid-cols-12 gap-4 md:gap-8 items-start px-2 md:px-4 cursor-default transition-all duration-500 ${
            ...
            >
              ...
              <div className="col-span-10 md:col-span-3">
                <h3 className={`font-mono text-lg md:text-xl font-light transition-colors duration-500 ${
                  hoveredStage === i ? "text-accent" : "text-white"
                }`}>
                  {stage.name}
                </h3>
              </div>
