...
const ProjectBetaSection = () => {
  ...
  return (
    <section id="project-beta" ref={ref} className="relative py-16 md:py-24 texture-crosshatch">
      ...
      <div className="relative max-w-[1400px] mx-auto px-6 md:px-8">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 md:gap-10 mb-8">
          <motion.div initial={{ opacity: 0 }} animate={inView ? { opacity: 1 } : {}} className="lg:col-span-7">
            <div className="flex items-center gap-3 mb-3">
              <div className="w-2 h-2 bg-accent/60 rotate-45 animate-pulse" />
          ...
          >
            ...
            <svg viewBox="0 0 100 100" className="w-full h-full" preserveAspectRatio="xMidYMid meet">
              <path
                ...
                fill="none" stroke="hsl(160, 82%, 61%)" strokeWidth="0.3" opacity="0.3"
              />
              <path
                d="M30,10 L45,8 L55,5 L68,8 L75,12 L80,20 L82,30 L78,38 L72,42 L70,50 L68,55 L65,58 L62,62 L60,65 L58,68 L55,72 L52,75 L48,78 L44,80 L40,82 L36,80 L33,76 L30,72 L28,68 L26,62 L25,55 L24,48 L23,42 L22,35 L24,28 L26,22 L28,16 Z"
                fill="hsl(160, 82%, 61%)" opacity="0.03"
              />
              ...
              {networks.map((n, i) => {
                ...
                return (
                  <g key={i} onMouseEnter={() => setHoveredNetwork(i)} onMouseLeave={() => setHoveredNetwork(null)} className="cursor-pointer">
                    <circle cx={n.cx} cy={n.cy} r={isHovered ? 4 : 2.5} fill="none" stroke="hsl(160, 82%, 61%)" strokeWidth="0.3" opacity={isHovered ? 0.8 : 0.4}>
                    ...
                    <circle cx={n.cx} cy={n.cy} r={isHovered ? 2 : 1.2} fill="hsl(160, 82%, 61%)" opacity={isHovered ? 1 : 0.7} className="transition-all duration-300" />
                    {isHovered && (
                      <g>
                        <rect x={n.cx + 3} y={n.cy - 5} width={28} height={10} rx="0.5" fill="hsl(0, 0%, 5%)" stroke="hsl(160, 82%, 61%)" strokeWidth="0.2" opacity="0.95" />
                        <text x={n.cx + 5} y={n.cy - 0.5} fill="hsl(160, 82%, 61%)" fontSize="2.5" fontFamily="monospace">{n.pop} · {n.metric}</text>
                      </g>
                    )}
                  </g>
                );
              ...
              {networks.map((n, i) =>
                networks.slice(i + 1).map((m, j) => (
                  <line key={`${i}-${j}`} x1={n.cx} y1={n.cy} x2={m.cx} y2={m.cy} stroke="hsl(160, 82%, 61%)" strokeWidth="0.15" opacity="0.1" />
                ))
              )}
            </svg>
          ...
          <div>
            <div className="border-t border-white/[0.06]">
              {networks.map((network, i) => (
                <motion.div
                  ...
                  className={`py-4 px-4 border-b border-white/[0.06] transition-all duration-300 cursor-default ${
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-6">
                      ...
                      <span className="inline-flex items-center gap-2 font-mono text-xs tracking-[0.15em] uppercase text-accent/70">
                        <span className="w-1.5 h-1.5 rotate-45 bg-accent/60 animate-pulse" />
                        ACTIVE
                      </span>
                    </div>
                  </div>
            ...
            >
              {[
                ...
                <div key={i} className="bg-background/80 p-4 md:p-5 panel-3d">
